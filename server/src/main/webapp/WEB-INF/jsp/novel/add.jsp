        ...
        <form id="fromId" name="formName" method="post" enctype="multipart/form-data"
            ...
            <div class="block-area">
                ...
                <div class="row">
                    ...
                    <%--<div class="col-md-6 m-b-15">
                        <label>选择主创人员：</label>
                        <select  id="creatorList" onclick="" value="${co.name}" style="width: 150px;" class="select"></select>
                        &lt;%&ndash;<div class="col-sm-2">
                            <select>
                                <c:forEach var="co" items="${creatorList}">
                    ...
                    <div class="col-md-6 m-b-15" style="margin-top: 10px">
                        <label>选择主创人员：</label>
                        <select  id="creatorList" onchange="novel.fn.addCreator();" class="select"></select>
                    </div>
                    <hr class="whiter m-t-20"/>
                        ...
                        <c:if test="${null != workCreatorList && workCreatorList.size() > 0 }">
                            <c:forEach var="co" items="${workCreatorList}">
                                <div id="ul${co.id }" class="col-md-1 m-b-15">
                                    ...
                                    <a id="a" href="javascript:void(0);" style="float: none; z-index: 10; position: relative; bottom: 30px; left: 106px;" class="axx" onclick="novel.fn.delete(${co.id}, ${co.userInfo.id})">
                    ...
                    <div class="col-md-1 m-b-15" style="display: none" id="addCreator">
                        ...
                        <button type="button" onclick="novel.fn.deleteCreator(this)" class="btn btn-info btn-sm m-t-10">删除</button>
                    </div>
                    <hr class="whiter m-t-20"/>
                    <div class="col-md-12 m-b-15" style="margin-top: 10px;">
                        <label>小说简介：</label>
                            ...
                            <c:if test="${null != workSurroundList && workSurroundList.size() > 0 }">
                                <c:forEach var="co" items="${workSurroundList}">
                                    <div class="col-md-6 m-b-15" id="ul${co.id }">
                                        ...
                                        <button type="button" onclick="novel.fn.deletes(${co.id});" class="btn btn-info btn-sm m-t-10">删除</button>
                                    </div>
                                </c:forEach>
                            </c:if>
                        </div>
                        <div class="col-md-12 m-b-15">
                            <button type="button" onclick="novel.fn.do();" class="btn btn-info btn-sm m-t-10">增加</button>
                        </div>
    ...
    novel = {
        ...
        fn: {
            ...
            checkData: function () {
                ...
                if (null == name || name == '') {
                    $common.fn.notify("请输入小说名称", "error");
                    flag = false;
                    return;
                }

                if (name.length > 20) {
                    $common.fn.notify("小说名称字数最多20字", "error");
                    flag = false;
                    return;
                }

                if (null == seriesCount || seriesCount == '') {
                    $common.fn.notify("请输入小说总回数", "error");
                    flag = false;
                    return;
                }

                if (isNaN(seriesCount)) {
                    $common.fn.notify("小说总回数必须为数字", "error");
                    flag = false;
                    return;
                }

                if (null == category || category == '') {
                    $common.fn.notify("请输入小说分类", "error");
                    flag = false;
                    return;
                }

                if (null == description || description == '') {
                    $common.fn.notify("请输入小说简介", "error");
                    flag = false;
                    return;
                }

                if (description.length > 1000) {
                    $common.fn.notify("小说简介字数最多1000字", "error");
                    flag = false;
                    return;
                }
            ...
            save: function () {
                if (novel.fn.checkData()) {
                    console.log($('#creatorList option:selected').val());
                    $("#fromId").ajaxSubmit({
                        url: "${contextPath}/admin/novel/save",
                        type: "POST",
                        data: {
                        ...
                        success: function (result) {
                            if (result == 1) {
                                ...
                                window.location.href = "${contextPath}/admin/novel/index";
                            } else {
            ...
            getSelectList: function () {
                ...
                $leoman.ajax("${contextPath}/admin/novel/creatorList", null, function (result) {
                    if (null != result) {
                        $('#creatorList').empty();
