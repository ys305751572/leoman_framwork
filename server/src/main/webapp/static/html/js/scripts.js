...
(function() {
    ...
    $("#side-menu").find("li").each(function() {
        var menus = $(this).find("a").each(function() {
            var aUrl = $(this).prop("href");
            var pageHeadName  = $("section .panel-heading");
            var url = pageHeadName.context.URL;
            var _index = url.lastIndexOf("/");
            url = url.substring(0,_index);

            var _index2 = aUrl.lastIndexOf("/");
            aUrl = aUrl.substring(0, _index2);
            if(aUrl == url) {
                $(this).parent("li").addClass("active");
                var ulMenu = $(this).parent().parent("ul .sub-menu-list");
                if(ulMenu.length > 0) {
                    ulMenu.parent("li").addClass("nav-active");
                }
            }
        });
    });
